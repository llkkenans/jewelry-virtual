import { NextRequest, NextResponse } from 'next/server'
import { supabaseAdmin } from '@/lib/supabase/server'
import { getPresignedUrl, uploadToR2 } from '@/lib/r2'
import { runFashn } from '@/lib/fashn-client'

export const maxDuration = 60

const TRYON_MAX_CREDIT_COST = 4

export async function POST(req: NextRequest) {
  // Auth
  const token = req.headers.get('authorization')?.replace('Bearer ', '')
  if (!token) {
    return NextResponse.json({ error: 'Yetkisiz' }, { status: 401 })
  }

  const { data: { user }, error: authError } = await supabaseAdmin.auth.getUser(token)
  if (authError || !user) {
    return NextResponse.json({ error: 'Yetkisiz' }, { status: 401 })
  }

  // Credit check
  const { data: profile } = await supabaseAdmin
    .from('profiles')
    .select('credits')
    .eq('id', user.id)
    .single()

  if (!profile || profile.credits < TRYON_MAX_CREDIT_COST) {
    return NextResponse.json({ error: 'Yetersiz kredi' }, { status: 403 })
  }

  // Parse body
  let garmentImage: string, userModelId: string
  try {
    const body = await req.json()
    garmentImage = body.garmentImage
    userModelId = body.userModelId
  } catch {
    return NextResponse.json({ error: 'Geçersiz JSON gövdesi' }, { status: 400 })
  }

  if (!garmentImage || !userModelId) {
    return NextResponse.json(
      { error: 'Eksik parametre: garmentImage ve userModelId gerekli' },
      { status: 400 }
    )
  }

  // Fetch user model — validates ownership via user_id check
  const { data: userModel, error: modelError } = await supabaseAdmin
    .from('user_models')
    .select('id, image_key')
    .eq('id', userModelId)
    .eq('user_id', user.id)
    .single()

  if (modelError || !userModel) {
    return NextResponse.json({ error: 'Model bulunamadı' }, { status: 404 })
  }

  // Get presigned URL for user's model image
  let modelImageUrl: string
  try {
    modelImageUrl = await getPresignedUrl(userModel.image_key as string, 3600)
  } catch (err) {
    console.error('R2 presign failed for model:', userModel.image_key, err)
    return NextResponse.json({ error: 'Model görseli alınamadı' }, { status: 500 })
  }

  // Call FASHN tryon-max
  let result
  try {
    result = await runFashn('tryon-max', {
      model_image: modelImageUrl,
      garment_image: garmentImage,
    })
  } catch (err) {
    console.error('FASHN tryon-max error:', err)

    await supabaseAdmin
      .from('clothing_generations')
      .insert({
        user_id: user.id,
        category: 'tryon-max',
        status: 'failed',
        credits_used: 0,
      })

    return NextResponse.json({ error: 'Üretim başarısız oldu. Lütfen tekrar deneyin.' }, { status: 502 })
  }

  if (!result.output || result.output.length === 0) {
    return NextResponse.json({ error: 'Çıktı üretilemedi' }, { status: 500 })
  }

  // Download output and store in R2
  const fashnOutputUrl = result.output[0]
  let outputUrl: string

  try {
    const outputResponse = await fetch(fashnOutputUrl)
    const outputBuffer = Buffer.from(await outputResponse.arrayBuffer())
    const outputKey = `outputs/${user.id}/fashion/${Date.now()}-${Math.random().toString(36).slice(2)}.png`
    await uploadToR2(outputBuffer, outputKey, 'image/png')
    outputUrl = await getPresignedUrl(outputKey, 3600)
  } catch (err) {
    console.error('R2 upload failed, returning FASHN URL:', err)
    outputUrl = fashnOutputUrl
  }

  // Save to DB
  const { data: genRecord, error: dbError } = await supabaseAdmin
    .from('clothing_generations')
    .insert({
      user_id: user.id,
      category: 'tryon-max',
      status: 'done',
      credits_used: TRYON_MAX_CREDIT_COST,
      output_image_url: outputUrl,
      is_saved: false,
    })
    .select('id')
    .single()

  if (dbError) {
    console.error('DB insert error:', dbError)
  }

  return NextResponse.json({
    outputUrl,
    generationId: genRecord?.id ?? null,
    fashnId: result.id,
  })
}
