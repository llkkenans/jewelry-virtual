...
function calculateEditCredits(resolution: Resolution = '1k'): number {
...
export async function POST(req: NextRequest) {
  ...
  // Parse body
  let sourceImageUrl: string, prompt: string, resolution: Resolution
  try {
  ...
  if (!sourceImageUrl || !prompt) {
    return NextResponse.json(
      { error: 'Eksik parametre: sourceImageUrl ve prompt gerekli' },
      { status: 400 }
    )
  }

  const creditCost = calculateEditCredits(resolution)
  ...
  let result
  try {
    result = await runFashn('edit', { image: sourceImageUrl, prompt, resolution })
  } catch (err) {
    console.error('FASHN edit error:', err)
    ...
    return NextResponse.json({ error: 'Düzenleme başarısız oldu. Lütfen tekrar deneyin.' }, { status: 502 })
  }
  ...
  try {
    ...
    const outputKey = `outputs/${user.id}/fashion/${Date.now()}-edit.png`
