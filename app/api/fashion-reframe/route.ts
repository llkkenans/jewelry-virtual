import { NextRequest, NextResponse } from 'next/server'
import { supabaseAdmin } from '@/lib/supabase/server'
import { uploadToR2, getPresignedUrl } from '@/lib/r2'
import { runFashn } from '@/lib/fashn-client'

export const maxDuration = 60

type Resolution = '1k' | '2k' | '4k'

function calculateReframeCredits(resolution: Resolution = '1k'): number {
  const costs: Record<Resolution, number> = { '1k': 1, '2k': 2, '4k': 3 }
  return costs[resolution]
}

export async function POST(req: NextRequest) {
  // Auth
  const token = req.headers.get('authorization')?.replace('Bearer ', '')
  if (!token) {
    return NextResponse.json({ error: 'Yetkisiz' }, { status: 401 })
  }

  const { data: { user }, error: authError } = await supabaseAdmin.auth.getUser(token)
  if (authError || !user) {
    return NextResponse.json({ error: 'Yetkisiz' }, { status: 401 })
  }

  // Parse body
  let sourceImageUrl: string, aspectRatio: string, resolution: Resolution
  try {
    const body = await req.json()
    sourceImageUrl = body.sourceImageUrl
    aspectRatio = body.aspectRatio
    resolution = (body.resolution as Resolution) ?? '1k'
  } catch {
    return NextResponse.json({ error: 'Geçersiz JSON gövdesi' }, { status: 400 })
  }

  if (!sourceImageUrl || !aspectRatio) {
    return NextResponse.json(
      { error: 'Eksik parametre: sourceImageUrl ve aspectRatio gerekli' },
      { status: 400 }
    )
  }

  const creditCost = calculateReframeCredits(resolution)

  // Credit check
  const { data: profile } = await supabaseAdmin
    .from('profiles')
    .select('credits')
    .eq('id', user.id)
    .single()

  if (!profile || profile.credits < creditCost) {
    return NextResponse.json({ error: 'Yetersiz kredi' }, { status: 403 })
  }

  // Call FASHN reframe
  let result
  try {
    result = await runFashn('reframe', {
      image: sourceImageUrl,
      aspect_ratio: aspectRatio,
      resolution,
    })
  } catch (err) {
    console.error('FASHN reframe error:', err)

    await supabaseAdmin
      .from('clothing_generations')
      .insert({
        user_id: user.id,
        category: 'reframe',
        status: 'failed',
        credits_used: 0,
      })

    return NextResponse.json({ error: 'Yeniden çerçeveleme başarısız oldu. Lütfen tekrar deneyin.' }, { status: 502 })
  }

  if (!result.output || result.output.length === 0) {
    return NextResponse.json({ error: 'Çıktı üretilemedi' }, { status: 500 })
  }

  // Download output and store in R2
  const fashnOutputUrl = result.output[0]
  let outputUrl: string

  try {
    const outputResponse = await fetch(fashnOutputUrl)
    const outputBuffer = Buffer.from(await outputResponse.arrayBuffer())
    const outputKey = `outputs/${user.id}/fashion/${Date.now()}-reframe.png`
    await uploadToR2(outputBuffer, outputKey, 'image/png')
    outputUrl = await getPresignedUrl(outputKey, 3600)
  } catch (err) {
    console.error('R2 upload failed, returning FASHN URL:', err)
    outputUrl = fashnOutputUrl
  }

  // Save to DB
  const { data: genRecord, error: dbError } = await supabaseAdmin
    .from('clothing_generations')
    .insert({
      user_id: user.id,
      category: 'reframe',
      status: 'done',
      credits_used: creditCost,
      output_image_url: outputUrl,
      is_saved: false,
    })
    .select('id')
    .single()

  if (dbError) {
    console.error('DB insert error:', dbError)
  }

  return NextResponse.json({
    outputUrl,
    generationId: genRecord?.id ?? null,
    fashnId: result.id,
  })
}
